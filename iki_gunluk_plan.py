...
sali = [ { "isim": 'gelecek haftaya hazirlik', "sure": 240, }, { "isim": 'ornek cozumlerine devam et', "sure": 180, }, { "isim": 'kahve molasi',"sure": 10, }, {"isim": 'kitap oku', "sure": 200, }, { "isim": 'spor yap', "sure": 40, }, ]
print("Pazartesi Puani: {}".format (reduce(lambda x,y:x+y,(map(lambda x:(x*20).__round__(),(filter(lambda x:x>=2,(map(lambda x:(list(x.values())[1]/60),pzt)))))))))
print("Sali Puani: {}".format(reduce(lambda x,y:x+y,(map(lambda x:(x*20).__round__(),(filter(lambda x:x>=2,(map(lambda x:(list(x.values())[1]/60),sali)))))))))
